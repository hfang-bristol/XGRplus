...
We are pleased to introduce an updated version of our XGR software, which has gained increasing popularity in genomics research since its first release (see [Genome Medicine 2016](https://www.ncbi.nlm.nih.gov/pubmed/27964755)). This update features a newly designed web server, called XGR Plus or XGR+, now made available [here](http://www.genomicsummary.pro/XGRplus) to all users for free and without any login requirements. It enables almost real-time enrichment and subnetwork analyses for user-input lists of genes, SNPs, genomic regions, or protein domains, leveraging ontologies, networks, and mangy other knowledgebase resources, such as e/pQTL, promoter capture Hi-C, and enhancer-gene maps.
...
- [ENRICHMENT ANALYSER (GENES) - EAG], identifies ontology terms enriched for the input gene list; see [Example Output](/app/examples/_tmp_RMD_EAgene.html)

- [ENRICHMENT ANALYSER (SNPS) - EAS], identifies ontology terms enriched for genes linked from the input SNP list; see [Example Output](/app/examples/_tmp_RMD_EAsnp.html)

- [ENRICHMENT ANALYSER (REGIONS) - EAR], identifies ontology terms enriched for genes linked from the input genomic region list; see [Example Output](/app/examples/_tmp_RMD_EAregion.html)

- [ENRICHMENT ANALYSER (DOMAINS) - EAD], identifies ontology terms enriched for the input protein domain list; see [Example Output](/app/examples/_tmp_RMD_EAdomain.html)
...
c2 <- c("~9 seconds","~55 seconds","~20 seconds","~2 seconds","~47 seconds","~55 seconds","~23 seconds")
df_runtime <- tibble(Analysers=c1, 'Runtime (Server)'=c2)
knitr::kable(df_runtime, caption="A summary of the runtime (on the server side) per tool estimated using Google Chrome.", row.names=FALSE, align='c')
```


# LANDING FRONTPAGE

```{r app-front, fig.cap="The landing frontpage of the XGR+ webserver.", fig.width=10, fig.height=5.5, echo=FALSE, eval=TRUE, fig.align="center",out.width="100%"}
library(png)
library(grid)
...
The error messages will be displayed, for example, if the input into the `SUBNETWORK ANALYSER (REGIONS)` is invalid (see the screenshot below). Notably, in the results page, a summary of input data is also returned to the users for reference.

<hr>

```{r SAregion-error, fig.cap="The screenshot for the error messages shown when the input is invalid, for example, in the `SUBNETWORK ANALYSER (REGIONS)` interface.", fig.width=10, fig.height=3.2, echo=FALSE, eval=TRUE, fig.align="center",out.width="100%"}
library(png)
library(grid)
...
- Under the `Output: subnetwork` tab, `A network visualisation` of the subnetwork is provided, along with `An interactive table` for the subnetwork genes.

```{r SAgene-results-output, fig.cap="Visualisation of the identified subnetwork, with genes/nodes color-coded by input gene significance information under the `Output: subnetwork` tab.", fig.width=10, fig.height=6.5, echo=FALSE, eval=TRUE, fig.align="center",out.width="95%"}
library(png)
library(grid)
...
- [Example Output](/app/examples/_tmp_RMD_SAsnp.html) includes interactive tables nd and high-quality figures for the resulting subnetwork. A summary of input data and the runtime (computed on the server side) is also returned for reference.
...
- Under the `Input SNP information` tab, `An interactive table` displays user-input summary data.

```{r SAsnp-results-input, fig.cap="`An interactive table` displaying user-input summary under the `Input SNP information` tab returned for exploration.", fig.width=10, fig.height=6.8, echo=FALSE, eval=TRUE, fig.align="center",out.width="95%"}
library(png)
library(grid)
...
```{r SAsnp-results-output, fig.cap="A network visualisation of the resulting subnetwork, with genes/nodes color-coded by linked gene scores (ranged from 1 to 10) under the `Output: subnetwork` tab.", fig.width=10, fig.height=7.7, echo=FALSE, eval=TRUE, fig.align="center",out.width="95%"}
library(png)
library(grid)
...
- Under the `Input genomic region information` tab, `An interactive table` lists user-input summary data.

```{r SAregion-results-input, fig.cap="User-input summary data under the `Input genomic regions information` tab returned for exploration.", fig.width=10, fig.height=7.2, echo=FALSE, eval=TRUE, fig.align="center",out.width="95%"}
library(png)
library(grid)
...
- Under the `Output: subnetwork` tab, `A network visualisation` of the identified subnetwork is provided, along with `An interactive table` for the subnetwork genes.

```{r SAregion-results-output, fig.cap="A network visualisation of the resulting subnetwork under the `Output: subnetwork` tab, with genes/nodes color-coded by linked gene scores, which range from 1 to 10.", fig.width=10, fig.height=7.7, echo=FALSE, eval=TRUE, fig.align="center",out.width="95%"}
library(png)
library(grid)
